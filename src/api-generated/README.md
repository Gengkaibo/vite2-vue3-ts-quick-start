...
# 工具地址 https://editor.swagger.io
# 注意事项
* @Date 2022年3月31日
* @Author 沉淀
### 1. 每次覆盖typescript-jquery-client文件夹后需要修改configuration.ts,将swagger-editor自动生成的不必要的参数注释掉
```ts
export class Configuration {
    apiKey: string; // 代表Authorization
    // username: string;
    // password: string;
    // accessToken: string | (() => string);
}
```
### 2. 在useApiGenerated.ts中添加输出
...
/*
swagger-editor自动生成的代码中的接口是默认的https, 所以这里需要重新配置ip为http, 刚好我在全局的env文件有配置服务的IP端口
*/
const ipPort = Base.IS_DEV? '/apis' : import.meta.env.VITE_APP_BASE_API as string

const API = {
  DictControllerApi: new tjc.DictControllerApi(ipPort, {
    apiKey: token
  },{
    // 这里写其他的ajax配置
    timeout: 5 * 1000
...
API.DictControllerApi.getListUsingPOST(params).then(res => {
  // do something resolved
}).catch(e=>{
  // do something rejected
})
```
### 4. 参数列表太长解决方案
* 协调后端人员将get接口全部改为post接口
* 自行改造get接口，将参数列表进行对象化
### 5. 其他的ajax配置
```ts
/**
